...
class FileListGenerator(QWidget):
    ...
    def copy_and_generate_folder(self, com_dirname, org_dirname, file_dirname, folder_name):
        try:
            ...
            unc_path = r'\\?\{}'.format(file_dirname)
            shutil.copy2(unc_path, output_dir)
    ...
    def generate_metadata(self):
        ...
        if is_excel_exist is False:
            ...
            headers = ['위원회', '피감기관', 'BOOK_ID', 'SEQNO', 'FILE_NAME',
                       '국정감사 파일명', '위원', '질의', '압축파일 이름', 'REALFILE_NAME', '실제 경로', '문서 종류', '압축파일 확인필요', '파일 크기', '확장자']
            ...
            if row['확장자'] == '.zip':
                ...
                try:
                    ...
                    ws.cell(row=last_row + index + 1 + tmp_idx, column=11,
                            value=row['실제 경로'])
                    norm_dir = os.path.normpath(row['전체 경로'])
                    norm_dir = r'\\?\{}'.format(norm_dir)
                    file_size = os.path.getsize(norm_dir)
                    ws.cell(row=last_row+index+1+tmp_idx,
                            column=14, value=file_size)
                except:
                    print(row['전체 경로'])
    ...
    def read_zip_file(self, ws, row, last_row, index, tmp_idx):
        try:
            tmp_idx_zip = -1
            row['전체 경로'] = os.path.join('\\\\?\\', row['전체 경로'])
            with zipfile.ZipFile(row['전체 경로'], 'r') as zip_ref:
                tmp_zip_file_list = zip_ref.infolist()
                zip_file_name_list = []
                zip_file_size_list = []
                ...
                for info in tmp_zip_file_list:
                    ...
                    if not filename.endswith('/'):
                        ...
                        zip_file_name_list.append(filename)
                        zip_file_size_list.append(filesize)
                zip_info_list = list(
                    zip(zip_file_name_list, zip_file_size_list))
                zip_info_list = natsorted(zip_info_list, key=lambda x: x[0])
                zip_file_name_list, zip_file_size_list = zip(*zip_info_list)
                zip_file_name_list = list(zip_file_name_list)
                zip_file_size_list = list(zip_file_size_list)

                for idx, (name, size) in enumerate(zip(zip_file_name_list, zip_file_size_list)):
                    if os.path.basename(name):
                        ...
                        ws.cell(row=last_row + index + 1 + idx + tmp_idx,
                                column=10, value=os.path.basename(name))
                        tmp_path = pathlib.Path(row['실제 경로']).with_suffix('')
                        zip_file_path = os.path.join(tmp_path, name)
                        ws.cell(row=last_row + index + 1 + idx + tmp_idx,
                                column=11, value=zip_file_path)
                        ws.cell(row=last_row + index + 1 + idx + tmp_idx,
                                column=14, value=size)
                        tmp_idx_zip += 1
            return tmp_idx_zip
            ...
            ws.cell(row=last_row + index + 1 +
                    tmp_idx, column=13, value='파일 찾을 수 없음')
            ws.cell(row=last_row + index + 1 + tmp_idx, column=10,
                    value=row['FILE_NAME'])  # 파일명
            ws.cell(row=last_row + index + 1 + tmp_idx, column=11,
                    value=row['실제 경로'])  # 실제 경로
