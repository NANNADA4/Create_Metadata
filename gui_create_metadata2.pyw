...
class FileListGenerator(QWidget):
    ...
    def __init__(self):
        ...
        self.file_require_answer = ['위원회 요구자료 - 답변']
    ...
    def select_root_folder(self):
        ...
        if self.root_folder and self.output_excel or self.root_folder and self.output_folder:
    ...
    def generate_metadata(self):
        ...
        if is_excel_exist is False:
            ...
            # 헤더 추가
            headers = ['위원회', '피감기관', '위원', 'REALFILE_NAME',
                       '실제 경로', '문서 종류', '이름', '배분날짜', '완료날짜']
            ...
            if matches2:
                if matches2[0] == '이용' and tmp_org != '문화체육관광위원회':
                    if len(matches2.groups()) > 1 and matches2[1]:
                        ws.cell(row=last_row + index + 1, column=3,
                                value=matches2[-1] + ' 위원')  # 위원
                    else:
                        ws.cell(row=last_row + index + 1,
                                column=3, value=None)  # 위원
                else:
                    ws.cell(row=last_row + index + 1, column=3,
                            value=matches2[0] + ' 위원')  # 위원
            else:
                # 실제 경로 폴더명에서 위원명이 검출되지 않을경우 공백
                ws.cell(row=last_row + index + 1, column=3, value=None)
            ws.cell(row=last_row + index + 1, column=4,
                    value=row['FILE_NAME'])  # 파일명
            ws.cell(row=last_row + index + 1, column=5,
